 ...
 * is at least one record. Each item links to its detail route. The copy is
 * deliberately neutral — a name and its base currency — with nothing implying
 * advice, performance, safety, or returns.
 */
    ...
    <ul className="portfolio-list" aria-label="Your portfolios">
      {portfolios.map((portfolio) => (
        <li key={portfolio.id} className="portfolio-list__item">
          <Link className="portfolio-list__link" to={`/portfolios/${portfolio.id}`}>
